# micsave — diff the live (mutable) EasyEffects preset against the copy in
# this repo and commit it back. Counterpart to the easyeffects module's
# copy-out-of-store activation (the preset is intentionally mutable).
{lib, ...}:
lib.custom.mkScript {
  name = "micsave";
  description = "MicSave EasyEffects preset commit tool";
  runtimeInputs = pkgs: with pkgs; [libnotify git delta jq coreutils];
  extraOptions = {
    presetGitPath = lib.mkOption {
      type = lib.types.str;
      description = "Absolute string path to the preset file in the git repo. Must be a string, not a Nix path, so the live repo file is used (and stays writable) instead of a store copy.";
    };
