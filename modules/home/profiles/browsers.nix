# Shared browser profile: Zen with a pinned extension set. Identical for all
# users that opt in via custom.profiles.browsers.enable.
{
  config,
  lib,
  ...
}: let
  cfg = config.custom.profiles.browsers;
in {
  options.custom.profiles.browsers.enable =
    lib.mkEnableOption "Zen browser with the standard extension set";
