# Shared gaming profile: Steam, Discord (+ Vencord plugins), arRPC, bolt,
# prismlauncher, and the Millennium plugins. Per-user deltas exposed as options.
{
  config,
  lib,
  pkgs,
...
in {
  options.custom.profiles.gaming = {
    enable = lib.mkEnableOption "Steam/Discord gaming profile";
    slsSteam = lib.mkEnableOption "SLSsteam integration for Steam";
    discordRpcNoctalia = lib.mkEnableOption "Noctalia Discord RPC integration";
    steamNiriLauncher = lib.mkEnableOption "Custom Steam desktop entry that focuses an existing niri window";
  };
  ...
  config = lib.mkIf cfg.enable {
    custom.programs = {
      ...
      discord-rpc-noctalia.enable = cfg.discordRpcNoctalia;
      bolt.enable = true;
      prismlauncher.enable = true;
    };

    # Custom Steam launcher that focuses an existing niri window instead of
    # spawning a second instance. Only meaningful under niri.
    xdg.desktopEntries.steam = lib.mkIf (cfg.steamNiriLauncher && config.custom.programs.niri.enable) {
      name = "Steam";
